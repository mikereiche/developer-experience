...
async function main() {
  ...
  app.get('/api/hotels/byid/:id?',
    ...
    console.log('Promise.all()!' || ids )
      const results = await Promise.all( 
          ids.map(
            async id  => {
          const doc = await hotels.get(id, {
            project: cols
          })
      ...
      return res.send({
        data: results,
        context: [`kv get hotel within ${id}`]
      })
    })
  )
  ...
  app.get('/api/hotels/:description/:location?',
    runAsync(async (req, res) => {
      ...
      //const result = await cluster.searchQuery('travel-sample.inventory.hotels-index', qp, { limit: 100 })
